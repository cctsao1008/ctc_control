...
#else
			char* dateLengthSpacing = malloc(strlen(date) + 1);
			// Used to ensure errno output is aligned correctly
			dateLengthSpacing[strlen(date) + 1];
			memset(dateLengthSpacing, ' ', strlen(date) + 1);
#endif
...
void writeStackTrace() {
	...
	void* backtrace_addresses = malloc(max_backtrace_size);
    // size of backtrace
...
void loadConfig( const char* config ) {
    ...
    for( int i = 0; i < (unsigned)strlen( settingsBuffer ); i++ ) {
        if( settingsBuffer[i] == '=' ) {
...
static char** getPrettyBacktrace( void* addresses[], int array_size ) {
    ...
    //char command_string[max_command_string_size];   // set to the maximum possible path size
	char* command_string = malloc(max_command_string_size);   // set to the maximum possible path size
    //char exe_path[max_path_size];
	char* exe_path = malloc(max_path_size);
